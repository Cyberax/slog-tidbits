...
func (p *PinpointLogLevels) sortPrefixes() {
	slices.SortFunc(p.prefixes, func(a, b locationPrefixEntry) bool {
		ln1 := len(a.LocationPrefix)
		ln2 := len(b.LocationPrefix)
		if ln1 != ln2 {
			return ln1 < ln2
		}
		return a.LocationPrefix < b.LocationPrefix
	})
...
func (p *PinpointLogLevels) LevelForPackage(pkg string) (slog.Level, bool) {
	p.mtx.Lock()
	defer p.mtx.Unlock()

	res, ok := p.levelCache[pkg]
	if ok {
		return res, true
	}
	...
	for _, curPrefix := range p.prefixes {
		if strings.HasPrefix(pkg, curPrefix.LocationPrefix) {
